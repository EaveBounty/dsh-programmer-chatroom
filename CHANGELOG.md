...
- 根治设置页闪回：资料编辑移到**模块级草稿存储**，任何重挂载/重渲染都不再丢失正在编辑的昵称/颜色。
- 修复「发送中」卡死：所有关键请求加超时，发送锁在 `finally` 中必定释放——主会话忙/请求挂起时也能恢复，不会一直挡发送。
- 移除设置里的「公网站点地址」入口（避免暴露攻击面）；站点基址由宿主内部默认（`chatroom.iloveljn.cn/machine_list`），UI 不再显示。
