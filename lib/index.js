...
const http=require('http'),https=require('https'),crypto=require('crypto'),os=require('os'),fs=require('fs'),dgram=require('dgram');
...
function colr(s){return /^#[0-9a-fA-F]{6}$/.test(String(s||''))?s:'#4D6BFE'}
function strip(s){s=String(s||'');while(s.length&&s.slice(-1)==='/')s=s.slice(0,-1);return s}
const BEACON_PORT=39400;
const netMode={mode:'lan',rendezvous:'',public:''};
const autoPeers=new Map(),rdvPeers=new Map(),beaconLog=new Map(),peerNameByUrl=new Map(),scanManual=new Set(),failByUrl=new Map();
let beaconSock=null,beaconTimer=null,lastAnnounce=0,lastRdvNodes=0,lastMergeTs=0,lastAntiEntropy=0,lastCpolarTs=0,lastTableSig=null,lanScanBusy=false,lanLastPass=0,antiPtr=0,netColdDone=false,stalePeers=[];
const messages=[],seen=new Map();let nextIdx=1;let manualPeers=args.peers.slice(),peers=[];const peerSince=new Map();
function recomputePeers(){const seenUrl={},out=[];function add(u){if(u&&!seenUrl[u]){seenUrl[u]=1;out.push(u);}}for(const m of manualPeers)add(m);for(const k of autoPeers.keys())add(k);for(const k of rdvPeers.keys())add(k);peers=out;}
...
const TABLE_FILE=(function(){const p=String(args.stateFile||'');const i=p.lastIndexOf('/');return (i>0?p.slice(0,i):'.')+'/table.json';})();
function startBeacon(){if(beaconSock)return;try{beaconSock=dgram.createSocket('udp4');beaconSock.on('error',function(){});beaconSock.on('message',function(msg,rinfo){try{const b=JSON.parse(String(msg));if(!b||b.t!=='dshc'||!b.httpPort||b.nodeId===name)return;const url='http://'+rinfo.address+':'+Number(b.httpPort);const isNew=!autoPeers.has(url);autoPeers.set(url,Date.now());if(isNew){peerSince.delete(url);recomputePeers();if(!beaconLog.has(b.nodeId)){console.log('[dshc-lan] discovered '+url);beaconLog.set(b.nodeId,Date.now());}}}catch(e){}});beaconSock.bind(BEACON_PORT,'0.0.0.0',function(){try{beaconSock.setBroadcast(true);}catch(e){}});beaconTimer=setInterval(function(){if(netMode.mode!=='lan')return;const payload=JSON.stringify({t:'dshc',nodeId:name,httpPort:args.port,lanIPs});const buf=Buffer.from(payload);try{beaconSock.send(buf,0,buf.length,BEACON_PORT,'255.255.255.255');}catch(e){}},3000);}catch(e){}}
function pruneLanPeers(){if(!autoPeers.size)return;const now=Date.now();let changed=false;for(const e of Array.from(autoPeers)){const u=e[0],t=e[1];if(now-t>25000){autoPeers.delete(u);peerSince.delete(u);if(scanManual.has(u)){scanManual.delete(u);manualPeers=manualPeers.filter(function(x){return x!==u});}peerNameByUrl.delete(u);changed=true;}}if(changed)recomputePeers();}
function outGet(url,timeoutMs,cb){let done=false;const fin=function(e,r,b){if(!done){done=true;cb(e,r,b);}};try{const mod=/^https:/i.test(String(url))?https:http;const req=mod.get(url,{timeout:timeoutMs||3000},function(res){let body='';res.setEncoding('utf8');res.on('data',function(c){if(body.length<(1<<20))body+=c;});res.on('end',function(){fin(null,res,body);});res.on('error',function(){fin(new Error('res'));});});req.on('timeout',function(){try{req.destroy();}catch(e){}fin(new Error('timeout'));});req.on('error',function(e){fin(e);});}catch(e){fin(e);}}
function outPost(url,obj,timeoutMs){let done=false;const fin=function(){if(!done){done=true;}};try{const body=Buffer.from(JSON.stringify(obj||{}));const mod=/^https:/i.test(String(url))?https:http;const req=mod.request(url,{method:'POST',timeout:timeoutMs||3000,headers:{'Content-Type':'application/json','Content-Length':body.length}},function(res){res.resume();res.on('end',fin);res.on('error',fin);});req.on('timeout',function(){try{req.destroy();}catch(e){}fin();});req.on('error',fin);req.write(body);req.end();}catch(e){fin();}}
function isPrivateIpLiteral(ip){const p=String(ip||'').split('.');if(p.length!==4)return false;for(const x of p){if(!x||!/^[0-9]{1,3}$/.test(x))return false;}const a=Number(p[0]),b=Number(p[1]);if(a>255||b>255||Number(p[2])>255||Number(p[3])>255)return true;if(a===10)return true;if(a===172&&b>=16&&b<=31)return true;if(a===192&&b===168)return true;if(a===127||a===0||a===169||a===100)return true;return false;}
function isPublicUrl(u){try{const o=new URL(String(u||''),'http://x');if(o.protocol==='https:')return true;return !isPrivateIpLiteral(o.hostname);}catch(e){return false;}}
function isSelfUrl(u){return !!netMode.public&&strip(netMode.public)===strip(String(u||''));}
function dropIfDown(url){if((failByUrl.get(url)||0)<3)return;if(!isPublicUrl(url)){failByUrl.delete(url);return;}if(!rdvPeers.has(url)&&manualPeers.indexOf(url)<0){failByUrl.delete(url);return;}rdvPeers.delete(url);manualPeers=manualPeers.filter(function(x){return x!==url});scanManual.delete(url);peerSince.delete(url);peerNameByUrl.delete(url);failByUrl.delete(url);recomputePeers();console.log('[dshc-net] drop down peer '+url);}
function probeHealth(url){return new Promise(function(resolve){outGet(strip(url)+'/health',3000,function(err,res,body){let j=null;if(!err&&res&&res.statusCode===200){try{const t=JSON.parse(body||'');if(t&&t.ok===true)j=t;}catch(e){}}if(j){failByUrl.delete(url);if(j.name)peerNameByUrl.set(url,String(j.name).slice(0,64));resolve(j);}else{failByUrl.set(url,(failByUrl.get(url)||0)+1);dropIfDown(url);resolve(null);}});});}
function tableUrls(){const seen={},arr=[];function add(u){const t=strip(u);if(t&&validPeer(t)&&!seen[t]){seen[t]=1;arr.push(t);}}for(const m of manualPeers)add(m);for(const k of autoPeers.keys())add(k);for(const k of rdvPeers.keys())add(k);arr.sort();return arr;}
function tableMd5(){return crypto.createHash('md5').update(tableUrls().join('\n')).digest('hex');}
function tableSize(){return tableUrls().length;}
function persistTable(){try{const urls=tableUrls();const sig=urls.join('\n');if(sig===lastTableSig)return;lastTableSig=sig;const md5=crypto.createHash('md5').update(sig).digest('hex');fs.writeFileSync(TABLE_FILE,JSON.stringify({ts:Date.now(),md5:md5,table:urls}));}catch(e){}}
function loadTableFile(){try{const t=JSON.parse(fs.readFileSync(TABLE_FILE,'utf8'));if(!t||!Array.isArray(t.table))return;const seen={};stalePeers=[];for(const u of t.table){const x=strip(u);if(x&&validPeer(x)&&!seen[x]){seen[x]=1;stalePeers.push(x);}}lastTableSig=stalePeers.join('\n');}catch(e){}}
loadTableFile();
function rdvAnnounce(){if(netMode.mode!=='net'||!netMode.rendezvous)return;const now=Date.now();if(now-lastAnnounce<60000)return;lastAnnounce=now;const base=strip(netMode.rendezvous);if(!base)return;outPost(base+'/announce',{deviceId:name,nodeId:name,name:name,httpUrl:netMode.public||null,lanIP:lanIPs[0]||null},3000);}
function rdvNodes(){if(netMode.mode!=='net'||!netMode.rendezvous)return;const now=Date.now();if(now-lastRdvNodes<8000)return;lastRdvNodes=now;const base=strip(netMode.rendezvous);if(!base)return;outGet(base+'/nodes?alive=90',5000,function(err,res,body){if(err||!res||res.statusCode!==200)return;try{const d=JSON.parse(body||'');const arr=(d&&Array.isArray(d.nodes))?d.nodes:[];const now2=Date.now();let changed=false;for(const n of arr){if(!n||n.deviceId===name||n.nodeId===name)continue;const u=strip(String(n.httpUrl||n.httpsUrl||''));if(!validPeer(u)||isSelfUrl(u))continue;if(!rdvPeers.has(u))peerSince.delete(u);rdvPeers.set(u,now2);}for(const e of Array.from(rdvPeers)){const u=e[0],t=e[1];if(now2-t>120000){rdvPeers.delete(u);peerSince.delete(u);changed=true;}}if(changed||arr.length)recomputePeers();}catch(e){}});}
function rdvMerge(){if(netMode.mode!=='net'||!netMode.rendezvous)return;const now=Date.now();if(now-lastMergeTs<600000)return;lastMergeTs=now;const base=strip(netMode.rendezvous);if(!base)return;const urls=tableUrls().filter(function(u){return isPublicUrl(u)&&!isSelfUrl(u)}).slice(0,40);if(!urls.length)return;const nodes=[];for(const u of urls){let host=u;try{host=new URL(u).hostname;}catch(e){}const nm=cleanNick(peerNameByUrl.get(u)||host);const seg=host.split('.');nodes.push({deviceId:nm,nodeId:nm,name:nm,httpUrl:u,lanIP:(seg.length===4&&seg.every(function(x){return /^[0-9]{1,3}$/.test(x)&&Number(x)<256;}))?host:null});}outPost(base+'/merge',{nodes:nodes},5000);}
function cpolarProbe(){if(netMode.mode!=='net')return;const now=Date.now();if(now-lastCpolarTs<15000)return;lastCpolarTs=now;outGet('http://127.0.0.1:9200/api/tunnels',1500,function(err,res,body){if(err||!res||res.statusCode!==200)return;try{const d=JSON.parse(body||'');const list=(d&&Array.isArray(d.tunnels))?d.tunnels:[];let best=null;for(const t of list){if(!t||String(t.public_url||'').slice(0,8)!=='https://')continue;const cfg=String((t.config&&t.config.addr)||'');if(cfg.indexOf(':'+args.port)>=0){best=t;break;}if(!best)best=t;}if(best&&best.public_url){const u=strip(best.public_url);if(netMode.public!==u){netMode.public=u;console.log('[dshc-net] cpolar public='+u);}}}catch(e){}});}
function netColdStart(){if(netMode.mode!=='net'||!netMode.rendezvous||netColdDone)return;netColdDone=true;const base=strip(netMode.rendezvous);if(!base)return;const cands=[];for(const u of stalePeers){const t=strip(u);if(validPeer(t)&&isPublicUrl(t)&&!isSelfUrl(t))cands.push(t);}let found=false;runPool(cands,probeHealth,3,function(u,j){if(j&&!found){found=true;rdvPeers.set(u,Date.now());if(manualPeers.indexOf(u)<0)manualPeers.push(u);recomputePeers();console.log('[dshc-net] cold-start stale peer alive '+u);}}).then(function(){if(found)return;outGet(base+'/bootstrap?alive=3600',6000,function(err,res,body){if(err||!res||res.statusCode!==200)return;try{const d=JSON.parse(body||'');const arr=(d&&Array.isArray(d.nodes))?d.nodes:(Array.isArray(d)?d:[]);const now=Date.now();let n=0;for(const nd of arr){if(!nd||nd.nodeId===name||nd.deviceId===name)continue;const u=strip(String(nd.httpUrl||nd.httpsUrl||''));if(!validPeer(u)||isSelfUrl(u))continue;rdvPeers.set(u,now);if(manualPeers.indexOf(u)<0)manualPeers.push(u);n++;}if(n){recomputePeers();console.log('[dshc-net] cold-start bootstrap '+n+' nodes');}}catch(e){}});});}
function antiEntropy(){if(netMode.mode!=='net')return;const now=Date.now();if(now-lastAntiEntropy<10000)return;lastAntiEntropy=now;const pool=tableUrls().filter(function(u){return isPublicUrl(u)&&!isSelfUrl(u)&&(failByUrl.get(u)||0)<3;});if(!pool.length)return;const picks=[];for(let k=0;k<3&&picks.length<pool.length;k++){const u=pool[antiPtr%pool.length];antiPtr++;if(picks.indexOf(u)<0)picks.push(u);}for(const u of picks)reconcilePeer(u);}
function reconcilePeer(u){probeHealth(u).then(function(j){if(!j)return;const theirMd5=j.tableMd5;if(theirMd5&&theirMd5!==tableMd5()){outGet(strip(u)+'/peers',4000,function(err,res,body){if(err||!res||res.statusCode!==200)return;try{const d=JSON.parse(body||'');if(!d||!Array.isArray(d.peers))return;const theirs=[];for(const x of d.peers){const t=strip(x);if(validPeer(t)&&!isSelfUrl(t)&&theirs.indexOf(t)<0)theirs.push(t);}const now=Date.now();let changed=false;for(const t of theirs){if(rdvPeers.has(t)||manualPeers.indexOf(t)>=0)continue;if(autoPeers.has(t)){autoPeers.set(t,now);continue;}rdvPeers.set(t,now);if(manualPeers.indexOf(t)<0)manualPeers.push(t);changed=true;}if(changed)recomputePeers();const missing=tableUrls().filter(function(t){if(theirs.indexOf(t)>=0||isSelfUrl(t)||!isPublicUrl(t))return false;return rdvPeers.has(t)||manualPeers.indexOf(t)>=0;}).slice(0,10);runPool(missing,probeHealth,4);}catch(e){}});}})}
function runPool(list,worker,conc,onEach){return new Promise(function(resolve){if(!list.length){resolve();return;}let i=0,act=0;const finish=function(){if(act===0&&i>=list.length)resolve();};const fill=function(){while(act<conc&&i<list.length){const item=list[i++];act++;Promise.resolve().then(function(){return worker(item);}).catch(function(){return null;}).then(function(res){act--;try{if(onEach)onEach(item,res);}catch(e){}fill();finish();});}finish();};fill();});}
function lanProbeHost(ip){const url='http://'+ip+':'+args.port;return new Promise(function(resolve){outGet(url+'/health',900,function(err,res,body){if(err||!res||res.statusCode!==200){resolve(null);return;}try{const j=JSON.parse(body||'');if(j&&j.ok===true){resolve({url:url,name:String(j.name||'').slice(0,64)});return;}}catch(e){}resolve(null);});});}
function lanScanFound(url,repName){const now=Date.now();const isNew=!autoPeers.has(url);if(manualPeers.indexOf(url)<0){manualPeers.push(url);scanManual.add(url);}autoPeers.set(url,now);if(repName&&peerNameByUrl.get(url)!==repName)peerNameByUrl.set(url,repName);failByUrl.delete(url);if(isNew){peerSince.delete(url);console.log('[dshc-lan] scan hit '+url+(repName?' name='+repName:''));}recomputePeers();}
function lanScanPass(){const nets=new Set();for(const ip of lanIPs){const p=String(ip).split('.');if(p.length===4)nets.add(p[0]+'.'+p[1]+'.'+p[2]+'.');}if(!nets.size)return Promise.resolve();const known=new Set(peers);const cands=[];for(const pre of nets){for(let h=1;h<=254;h++){const ip=pre+h;if(lanIPs.indexOf(ip)>=0)continue;const u='http://'+ip+':'+args.port;if(known.has(u)||autoPeers.has(u))continue;cands.push(ip);}}return runPool(cands,lanProbeHost,8,function(ip,hit){if(hit)lanScanFound(hit.url,hit.name);});}
function lanScanTick(){if(netMode.mode!=='lan'||lanScanBusy)return;const now=Date.now();if(now-lanLastPass<20000)return;lanLastPass=now;lanScanBusy=true;lanScanPass().catch(function(){}).then(function(){lanScanBusy=false;});}
function drainOutbox(){if(!args.outboxFile)return;let txt;try{txt=fs.readFileSync(args.outboxFile,'utf8');}catch(e){return;}const lines=String(txt||'').split('\n').filter(Boolean);for(const line of lines){try{const f=JSON.parse(line);if(f&&f.msgId&&!seen.has(f.msgId)){push({msgId:f.msgId,nodeId:f.nodeId||name,deviceId:String(f.deviceId||''),nick:cleanNick(f.nick),color:colr(f.color),text:cleanText(f.text),ts:Number(f.ts)||Date.now()});}}catch(e){}}if(lines.length){try{fs.writeFileSync(args.outboxFile,'');}catch(e){}}}
function writeState(){if(!args.stateFile)return;try{const payload=JSON.stringify({nodeId:name,messages:messages.map(function(m){return {idx:m.idx,msgId:m.msgId,nodeId:m.nodeId,deviceId:m.deviceId,nick:m.nick,color:m.color,text:m.text,ts:m.ts}}),lastIdx:nextIdx-1,peers:peers,lanIPs:lanIPs});fs.writeFileSync(args.stateFile,payload);}catch(e){}}
function readPeersFile(){if(!args.peersFile)return;let txt;try{txt=fs.readFileSync(args.peersFile,'utf8');}catch(e){return;}const list=String(txt||'').split('\n').map(function(s){return strip(s.trim());}).filter(Boolean);const merged=[];for(const u of list)if(!merged.includes(u))merged.push(u);manualPeers=merged;recomputePeers();}
...
if(path==='/peers'&&req.method==='GET'){res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({peers:peers}));return;}
if(path==='/health'){res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({ok:true,name:name,port:args.port,lanIPs:lanIPs,peers:peers,messageCount:messages.length,lastIdx:nextIdx-1,httpUrl:netMode.public||null,tableMd5:tableMd5(),tableSize:tableSize()}));return;}
...
if(path==='/peers'&&req.method==='POST'){collectBody(req,function(err,body){try{if(err){res.writeHead(413);res.end('too large');return;}const p=JSON.parse(body);const url=strip(String(p.url||''));if(p.action==='add'&&validPeer(url)&&manualPeers.indexOf(url)<0){manualPeers.push(url);peerSince.delete(url);}else if(p.action==='remove'&&url){manualPeers=manualPeers.filter(function(x){return x!==url});autoPeers.delete(url);rdvPeers.delete(url);scanManual.delete(url);peerSince.delete(url);peerNameByUrl.delete(url);}recomputePeers();res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({peers:peers}));}catch(e){res.writeHead(400);res.end('bad json');}});return;}
if(path==='/inject'&&req.method==='POST'){collectBody(req,function(err,body){try{if(err){res.writeHead(413);res.end('too large');return;}const frames=JSON.parse(body);const list=Array.isArray(frames)?frames:[frames];const accepted=[];for(const f of list){const msgId=String(f.msgId||'');if(!msgId||seen.has(msgId))continue;accepted.push(push({msgId:msgId,nodeId:String(f.nodeId||name),deviceId:String(f.deviceId||''),nick:cleanNick(f.nick),color:colr(f.color),text:cleanText(f.text),ts:Number(f.ts)||Date.now()}));}res.writeHead(200,{'Content-Type':'application/json'});res.end(JSON.stringify({accepted:accepted.length,lastIdx:nextIdx-1}));}catch(e){res.writeHead(400);res.end('bad json');}});return;}
res.writeHead(404);res.end('not found');}catch(e){try{res.writeHead(500);res.end('err');}catch(_){}}});
function poll(){try{readNetmode();pruneLanPeers();lanScanTick();rdvAnnounce();rdvNodes();antiEntropy();rdvMerge();netColdStart();readPeersFile();drainOutbox();writeState();persistTable();for(const b0 of peers.slice()){const since=peerSince.get(b0)||0;const url=strip(b0)+'/dsh-chat-relay?since='+since;outGet(url,6000,function(err,res,b){if(err||!res||res.statusCode!==200)return;try{const body=JSON.parse(b||'');if(body&&Array.isArray(body.messages)){for(const m of body.messages){if(!m.msgId||seen.has(m.msgId))continue;push({msgId:m.msgId,nodeId:String(m.nodeId||b0),deviceId:String(m.deviceId||''),nick:cleanNick(m.nick),color:colr(m.color),text:cleanText(m.text),ts:Number(m.ts)||Date.now()});}if(typeof body.lastIdx==='number')peerSince.set(b0,body.lastIdx);}}catch(e){}});}}catch(e){}}
setInterval(poll,2000);poll();
server.listen(args.port,args.bind,function(){recomputePeers();writeState();drainOutbox();readNetmode();startBeacon();console.log('[dshc-relay] up name=%s bind=%s:%d peers=%d lanIPs=%s state=%s',name,args.bind,args.port,peers.length,lanIPs.join(','),args.stateFile||'off');});
...
const apply = (ctx) => {
  ...
  let lanIP = null
  // Public internet address for this node (cpolar). The user runs cpolar in their OWN terminal
  // and pastes the resulting https URL back here; the plugin does NOT spawn cpolar (spawning
  // froze / hit permission issues). `tunnelUrl` = the pasted public URL.
  let netmode = 'lan'
  let rendezvous = ''
  let netmodePath = null
  let tunnelUrl = ''
  // Default rendezvous base (the owner's Cloudflare Workers site). Kept server-side so the
  // user never has to type/expose a site URL in the UI (avoids a visible attack surface).
  const DEFAULT_RDV = 'https://chatroom.iloveljn.cn/machine_list'
  const MODEFILE = '.dshc-chatmode.json'
  function writeNetmode() { if (!netmodePath) return; const rdv = (rendezvous && rendezvous.length) ? rendezvous : DEFAULT_RDV; writeFile(netmodePath, 'mode=' + netmode + '\nrendezvous=' + rdv + '\npublic=' + (tunnelUrl || '') + '\n') }
  // Remember network mode + rendezvous + public url across DSH restarts.
  async function persistNetmode() { const p = homeRoot() + '/' + MODEFILE; writeFile(p, JSON.stringify({ mode: netmode, rendezvous, publicUrl: tunnelUrl })) }
  async function loadNetmode() { const txt = await readFile(homeRoot() + '/' + MODEFILE); if (!txt) return; try { const o = JSON.parse(txt); if (typeof o.publicUrl === 'string') tunnelUrl = String(o.publicUrl).slice(0, 300); if (typeof o.rendezvous === 'string') rendezvous = String(o.rendezvous).slice(0, 300); netmode = 'lan'; if (o.mode === 'net' && tunnelUrl) netmode = 'net' } catch (e) {} }

  function tunnelStatus() { return { running: !!tunnelUrl, publicUrl: tunnelUrl, port: relayPort } }
  function tunnelSet(u) {
    const s = String(u || '').trim()
    if (!s) { tunnelUrl = '' }
    else if (/^https?:\/\/[A-Za-z0-9.\-]+(?::\d+)?(?:\/[^\s]*)?$/i.test(s)) { tunnelUrl = s.slice(0, 300) }
    else return { running: false, error: '公网地址需为 http(s)://… 格式' }
    writeNetmode(); persistNetmode()
    return tunnelStatus()
  }
  function tunnelClear() { tunnelUrl = ''; writeNetmode(); persistNetmode(); return tunnelStatus() }
  ...
  async function tryStartRelay() {
    relayPort = 39321 // FIXED chat port for all machines (LAN scan + internet both use it)
    const base = homeRoot()
    relayDir = base + '/.dshc-relay-' + Math.random().toString(36).slice(2, 6)
      ...
      handler: (req, res) => {
        try {
          ...
          if (pathname === '/dsh-chat/net' && req.method === 'POST') {
            readJson(req, (a, err) => { if (err) return json(400, { err }); if (a.rendezvous !== undefined) rendezvous = sanitizeRendezvous(a.rendezvous); if (a.mode === 'net' && !tunnelUrl) { netmode = 'lan'; writeNetmode(); persistNetmode(); return json(200, { mode: 'lan', rendezvous, needPublic: true }) } if (a.mode === 'lan' || a.mode === 'net') netmode = a.mode; writeNetmode(); persistNetmode(); json(200, { mode: netmode, rendezvous }) })
            return
          }
          ...
          if (pathname === '/dsh-chat/tunnel' && req.method === 'POST') {
            readJson(req, (a, err) => { if (err) return json(400, { err }); if (a && a.action === 'set') json(200, tunnelSet(a.publicUrl)); else if (a && a.action === 'clear') json(200, tunnelClear()); else json(200, tunnelStatus()) })
            return
          }
  ...
  if (h && h.handle) {
    ...
    h.handle('chat/tunnel', (a) => { if (a && a.action === 'set') return tunnelSet(a.publicUrl); if (a && a.action === 'clear') return tunnelClear(); return tunnelStatus() })
    ...
    h.handle('chat/stats', () => ({ messageCount: messages.length, modCount, peerCount: peers.length, seq: nextIdx - 1, relay: !!relayPort, relayPort, tunnelRunning: !!tunnelUrl }))
  }
