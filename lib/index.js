...
const apply = (ctx) => {
  ...
  // Remember the network mode across DSH restarts (client also stores its own copy).
  async function persistNetmode() { const p = homeRoot() + '/' + MODEFILE; writeFile(p, JSON.stringify({ mode: netmode, rendezvous })) }
  async function loadNetmode() { const txt = await readFile(homeRoot() + '/' + MODEFILE); if (!txt) return; try { const o = JSON.parse(txt); if (o.mode === 'lan' || o.mode === 'net') netmode = o.mode; if (typeof o.rendezvous === 'string') rendezvous = String(o.rendezvous).slice(0, 300) } catch (e) {} }
  ...
  async function tunnelStart() {
    ...
    try {
      // Launch cpolar directly (a `node -e` wrapper with piped stdio fails inside the plugin
      // subprocess sandbox). Public-URL detection is done by the companion relay via cpolar's
      // local dashboard API (http://127.0.0.1:9200/api/tunnels), not by reading stdout here.
      let cpolarBin = 'cpolar'
      if (subprocess.resolveExecutable) { try { cpolarBin = await subprocess.resolveExecutable('cpolar') } catch (e) { cpolarBin = 'cpolar' } }
      const base = homeRoot()
      const handle = subprocess.spawn({
        argv: [cpolarBin, 'http', '127.0.0.1:' + relayPort],
        cwd: base,
        stdio: { stdin: 'ignore', stdout: { mode: 'ignore' }, stderr: { mode: 'ignore' } },
        graceMs: 3000
      })
      tunnelProc = handle
      console.log('[dshc] cpolar tunnel started on 127.0.0.1:' + relayPort)
      return { running: true, publicUrl: tunnelUrl, port: relayPort, note: 'cpolar 已启动；公网地址由伴生中继经本地 9200 探测' }
      ...
      handler: (req, res) => {
        try {
          ...
          if (pathname === '/dsh-chat/net' && req.method === 'POST') {
            readJson(req, (a, err) => { if (err) return json(400, { err }); if (a.mode === 'lan' || a.mode === 'net') netmode = a.mode; rendezvous = sanitizeRendezvous(a.rendezvous); writeNetmode(); persistNetmode(); json(200, { mode: netmode, rendezvous }) })
            return
          }
