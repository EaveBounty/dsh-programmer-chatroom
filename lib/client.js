...
const dshProgrammerChatroomFactory = (require) => {
  ...
  const LEGACY_ME_KEY = 'dshc.identity' // old single-profile store (deviceId migrated once)
  ...
  function saveProfile(key, p) { try { localStorage.setItem(key, JSON.stringify({ nick: String(p.nick || ''), color: p.color })) } catch (e) {} }
  ...
  function ensureDeviceId() {
    ...
    if (!id) {
      let legacy = null
      try { const raw = localStorage.getItem(LEGACY_ME_KEY); if (raw) { const o = JSON.parse(raw); if (o && o.deviceId) legacy = o } } catch (e) {}
      id = legacy ? String(legacy.deviceId) : genDeviceId()
      saveDeviceId(id)
    }
    return id
  }
...
`
  ...
  function ChatPanel() {
    const [state, setState] = useState({
      ...
      mode: loadLocalMode(), rendezvous: '', meReady: false, seeded: false,
      deviceId: '', lanNick: '', lanColor: '', inetNick: '', inetColor: '',
      pubDraft: '', siteDraft: '', sending: false, nickErr: ''
    })
    ...
    const me = state.meReady
      ? { deviceId: state.deviceId, nick: state.mode === 'net' ? (state.inetNick || '匿名') : (state.lanNick || '匿名'), color: state.mode === 'net' ? state.inetColor : state.lanColor }
      : { deviceId: state.deviceId || '', nick: '', color: '' }
    ...
    // Identity: stable deviceId + per-scope profiles. Reconcile the network mode with the
    // host (host wins, mirrored back into localStorage dshc.netmode) and register the
    // ACTIVE scope's identity with the host so we show up in the members list.
    useEffect(() => {
      if (!open) return
      const d = ensureDeviceId()
      let lp = loadProfile(LAN_PROF_KEY)
      if (!lp) { lp = defaultProfile(); saveProfile(LAN_PROF_KEY, lp) }
      let ip = loadProfile(NET_PROF_KEY)
      if (!ip) { ip = defaultProfile(); saveProfile(NET_PROF_KEY, ip) }
      // Seed the profile drafts EXACTLY ONCE (per open), and never while the user is
      // editing: the functional update checks the live state via the `seeded` flag.
      patch((s) => s.seeded ? s : { ...s, deviceId: d, lanNick: lp.nick, lanColor: lp.color, inetNick: ip.nick, inetColor: ip.color, seeded: true, meReady: true })
      let done = false
      const reg = (mode) => {
        const p = mode === 'net' ? ip : lp
        fetch('/dsh-chat/me', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ deviceId: d, nick: (p.nick || '').trim() || '匿名', color: p.color || '#4D6BFE' }) }).then(() => {}).catch(() => {})
      }
      // Host is authoritative for the current mode; prefer it over localStorage.
      fetch('/dsh-chat/net').then(r => r.json()).then(d2 => {
        if (done) return
        const hm = (d2 && d2.mode === 'net') ? 'net' : 'lan'
        const rv = (d2 && d2.rendezvous) || ''
        // Reflect the host rendezvous and seed the 站点地址 draft (only while the user has
        // not typed into it yet) — never clobber an in-progress edit.
        patch((s) => {
          const upd = { mode: hm, rendezvous: rv }
          if (rv && !(s.siteDraft && s.siteDraft.trim())) upd.siteDraft = rv
          return { ...s, ...upd }
        })
        saveLocalMode(hm)
        reg(hm)
      ...
      fetch('/dsh-chat/tunnel').then(r => r.json()).then(d3 => {
        if (done || !d3) return
        patch((s) => {
          const upd = { tunnel: d3 }
    ...
    useEffect(() => {
      ...
      const t2 = setInterval(() => { fetch('/dsh-chat/tunnel').then(r => r.json()).then(d => { if (d) patch({ tunnel: d }) }).catch(() => {}) }, 3000)
    ...
    const send = async () => {
      ...
      try {
        ...
        patch({ input: '', picker: null, sending: false })
      } catch (e) { patch({ status: '发送失败', sending: false }) }
    }
    const saveProfile = async () => {
      const scope = state.mode === 'net' ? 'inet' : 'lan'
      const scopeWord = state.mode === 'net' ? '互联网' : '局域网'
      const typed = ((scope === 'inet' ? state.inetNick : state.lanNick) || '').trim()
      ...
      try { const r = await fetch('/dsh-chat/members'); const d = await r.json(); if (Array.isArray(d.members)) members = d.members } catch (e) {}
      const dup = typed && members.some((mb) => mb && mb.deviceId && mb.deviceId !== state.deviceId && mb.nick === typed)
      if (dup) { patch({ status: '', nickErr: '该昵称在当前〈' + scopeWord + '〉已存在，请换一个' }); return }
      // Persist each scope separately, then register the ACTIVE scope with the host.
      const lan = { nick: (state.lanNick || '').trim() || '匿名', color: state.lanColor || '#4D6BFE' }
      const inet = { nick: (state.inetNick || '').trim() || '匿名', color: state.inetColor || '#4D6BFE' }
      saveProfile(LAN_PROF_KEY, lan); saveProfile(NET_PROF_KEY, inet)
      patch({ nickErr: '', status: '已保存', lanNick: lan.nick, lanColor: lan.color, inetNick: inet.nick, inetColor: inet.color })
      const act = scope === 'inet' ? inet : lan
      fetch('/dsh-chat/me', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ deviceId: state.deviceId, nick: act.nick, color: act.color }) }).then(() => {}).catch(() => {})
    }
    ...
    const setNetMode = (mode) => {
      ...
      saveLocalMode(m)
      fetch('/dsh-chat/net', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mode: m, rendezvous: state.rendezvous || '' }) }).then(r => r.json()).then(d => { if (d) { const hm = d.mode === 'net' ? 'net' : 'lan'; patch({ mode: hm, status: '' }); saveLocalMode(hm) } }).catch(() => patch({ status: '模式设置失败' }))
    }
    const saveTunnel = async () => {
      const url = (state.pubDraft || '').trim()
      if (!url) { patch({ status: '请先粘贴 cpolar 公网地址' }); return }
      try {
        const r = await fetch('/dsh-chat/tunnel', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'set', publicUrl: url }) })
        const d = await r.json()
    ...
    const clearTunnel = async () => {
      try {
        const r = await fetch('/dsh-chat/tunnel', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'clear' }) })
        const d = await r.json()
        ...
        fetch('/dsh-chat/net').then(r2 => r2.json()).then(d2 => {
          const hm = (d2 && d2.mode === 'net') ? 'net' : 'lan'
          patch({ mode: hm })
          saveLocalMode(hm)
        }).catch(() => {})
    ...
    const saveSite = async () => {
      const rv = (state.siteDraft || '').trim()
      try {
        const r = await fetch('/dsh-chat/net', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ rendezvous: rv, mode: state.mode === 'net' ? 'net' : 'lan' }) })
        const d = await r.json()
        const hm = (d && d.mode === 'net') ? 'net' : 'lan'
        patch({ mode: hm, rendezvous: rv, status: '站点地址已保存', nickErr: '' })
        saveLocalMode(hm)
      } catch (e) { patch({ status: '站点地址保存失败' }) }
    }
    ...
    const scopeWord = state.mode === 'net' ? '互联网' : '局域网'
    const activeColor = state.mode === 'net' ? (state.inetColor || '#4D6BFE') : (state.lanColor || '#4D6BFE')
    const cards = []
    // 1) Display
    ...
    cards.push(createElement('div', { key: 'c-me', className: 'dshc-card' },
      ...
      createElement('div', { className: 'dshc-grp' },
        ...
        createElement('div', { className: 'dshc-srow' }, createElement('label', null, '昵称'), createElement('input', { type: 'text', value: state.lanNick || '', placeholder: '输入局域网昵称', onInput: (e) => patch({ lanNick: e.target.value, nickErr: '' }) })),
        createElement('div', { className: 'dshc-srow', style: { alignItems: 'flex-start' } }, createElement('label', null, '颜色'), createElement('div', { className: 'dshc-swatches' }, AVATAR_COLORS.map((c) => createElement('span', { key: 'lc' + c, className: 'dshc-swatch' + ((state.lanColor || '#4D6BFE') === c ? ' on' : ''), style: { background: c }, onClick: () => patch({ lanColor: c }) }))))),
      createElement('div', { className: 'dshc-grp' + (inetLocked ? ' disabled' : '') },
        createElement('div', { className: 'dshc-grpt' }, '🌐 互联网 昵称 / 颜色'),
        createElement('div', { className: 'dshc-srow' }, createElement('label', null, '昵称'), createElement('input', { type: 'text', value: state.inetNick || '', placeholder: '输入互联网昵称', disabled: inetLocked, onInput: (e) => patch({ inetNick: e.target.value, nickErr: '' }) })),
        createElement('div', { className: 'dshc-srow', style: { alignItems: 'flex-start' } }, createElement('label', null, '颜色'), createElement('div', { className: 'dshc-swatches' }, AVATAR_COLORS.map((c) => createElement('span', { key: 'ic' + c, className: 'dshc-swatch' + ((state.inetColor || '#4D6BFE') === c ? ' on' : ''), style: { background: c }, onClick: () => { if (!inetLocked) patch({ inetColor: c }) } })))),
    ...
    cards.push(createElement('div', { key: 'c-tun', className: 'dshc-card' },
      ...
      createElement('div', { className: 'dshc-srow', style: { marginBottom: 0, marginTop: 10 } },
        createElement('label', null, '站点地址'),
        createElement('input', { type: 'text', value: state.siteDraft || '', placeholder: 'https://chatroom.iloveljn.cn/machine_list', onInput: (e) => patch({ siteDraft: e.target.value, status: '', nickErr: '' }) }),
        createElement('button', { className: 'dshc-save', onClick: saveSite }, '保存')),
      createElement('div', { className: 'dshc-hint' }, '站点地址（rendezvous）从群聊站点页面获取，形如 …/machine_list；互联网模式下所有成员将在此站点汇合。')))
