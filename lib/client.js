...
`
  ...
  function ChatPanel() {
    const [state, setState] = useState({
      ...
      info: null, tunnel: null, tunnelBusy: false, display: loadDisplay(),
      mode: loadLocalMode(), rendezvous: '', meReady: false,
      deviceId: '', lanNick: '', lanColor: '', inetNick: '', inetColor: '',
      sending: false, nickErr: ''
    })
    ...
    useEffect(() => {
      ...
      patch({ deviceId: d, lanNick: lp.nick, lanColor: lp.color, inetNick: ip.nick, inetColor: ip.color, meReady: true })
      let done = false
      const reg = (mode) => {
      ...
      fetch('/dsh-chat/net').then(r => r.json()).then(d2 => {
        ...
        patch({ mode: hm, rendezvous: (d2 && d2.rendezvous) || '' })
        saveLocalMode(hm)
        reg(hm)
      }).catch(() => { if (!done) reg(state.mode === 'net' ? 'net' : 'lan') })
      fetch('/dsh-chat/tunnel').then(r => r.json()).then(d3 => { if (!done && d3) patch({ tunnel: d3 }) }).catch(() => {})
      return () => { done = true }
    }, [open])
    ...
    const tunnelUp = !!(state.tunnel && state.tunnel.running)
    const inetLocked = !tunnelUp
    const scopeWord = state.mode === 'net' ? '互联网' : '局域网'
    ...
    cards.push(createElement('div', { key: 'c-me', className: 'dshc-card' },
      ...
      createElement('div', { className: 'dshc-grp' + (inetLocked ? ' disabled' : '') },
        ...
        inetLocked ? createElement('div', { className: 'dshc-hint' }, '🔒 启用内网穿透后可设置互联网昵称') : null),
    ...
    cards.push(createElement('div', { key: 'c-tun', className: 'dshc-card' },
      ...
      createElement('div', { style: { marginTop: 8 } }, createElement('button', { className: 'dshc-tunnelbtn ' + (tunnelUp ? 'on' : 'off'), onClick: toggleTunnel, disabled: state.tunnelBusy }, tunnelUp ? '■ 停止穿透' : '▶ 一键启动穿透')),
      tunnelUp ? createElement('div', { className: 'dshc-hint' }, '公网地址: ', createElement('span', { className: 'dshc-urllink' }, state.tunnel.publicUrl || '获取中…')) : createElement('div', { className: 'dshc-hint' }, '互联网模式必备：穿透后插件会把公网地址自动上报到站点。关闭群聊/插件会自动停止穿透。')))
