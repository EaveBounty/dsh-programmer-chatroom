...
`
  ...
  function ChatPanel(props) {
    const [state, setState] = useState({ open: false, msgs: [], modLog: [], peers: [], seq: 0, modId: 0, input: '', status: '', tab: 'chat', picker: null, peerInput: '', info: null, nick: '程序员' + Math.random().toString(36).slice(2, 5) })
    ...
    return createElement('div', { className: 'dshc-wrap' },
      createElement('div', { className: 'dshc-h' },
        ...
        createElement('button', { className: 'dshc-close', onClick: () => patch({ open: false }) }, '✕')),
      relayBar,
      body,
  ...
  const apply = (ctx) => {
    if (ctx.styles) { try { ctx.styles.insert(css) } catch (e) {} }
    if (!ctx.slots) return
    // styles may not exist on ctx; inject manually if available via document
    ...
    const toggle = () => { /* handled by panel state via a shared flag */ }
    ctx.slots.inject('shell.overlay', () => ctx.slots.register({ name: 'shell.overlay', id: 'dshc-chat-panel', order: 100 }, (props) => createElement(ChatPanel, {})))
    ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({ name: 'sidebar.footer.action', id: 'dshc-chat-toggle', order: 50, label: () => '群聊' }, (props) => createElement('button', { onClick: () => { /* no-op; panel is always mountable */ }, style: { border: '1px solid var(--dshc-border,#333)', borderRadius: 10, padding: '7px 11px', cursor: 'pointer', background: 'transparent', color: 'inherit' } }, '💬 群聊')))
  }
